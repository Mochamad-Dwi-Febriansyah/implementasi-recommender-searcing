    ...
    <div class="container d-flex justify-content-center">
      <div class="row col-md-8">  
        <div class="col-12 mt-4">
          <h1 class="mb-3">MOVIE RECOMMENDER SYSTEM WITH MACHINE LEARNING</h1> 
          <form class="d-flex mb-3" role="search" action="" method="POST">
            @csrf
            <select class="form-select me-2" name="movie" aria-label="Default select example"> 
              @if(isset($movie_lists))
              @foreach ($movie_lists as $item)
              <option value="{{ $item['name'] }}"
                @if(isset($selected_movie) && $selected_movie == $item['name']) 
                selected 
              @endif>
                {{ $item['name'] }}
              </option> 
